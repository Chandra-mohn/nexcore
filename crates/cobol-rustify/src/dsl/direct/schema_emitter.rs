...
use super::schema_fields::{count_elementary, has_structural_children, is_filler};
...
impl DirectDslEmitter for DirectSchemaEmitter {
    ...
    fn emit(&self, ctx: &DirectEmitterContext<'_>) -> Vec<DslFile> {
        ...
        if let Some(ref data_div) = ctx.cobol_program.data_division {
            ...
            // --- Strategy 5: Co-access grouping from hints ---
            if let Some(hints) = ctx.hints {
                let co_access = extract_co_access_schemas(
                    &data_div.working_storage,
                    hints,
                    &ctx.program_name,
                );
...
fn emit_file_schemas(
    ...
    for record in &fd.records {
        ...
        // Build fields and nested objects from COBOL level hierarchy
        let (mut flat_fields, mut flat_constraints, nested_objects) =
            super::schema_fields::build_nested_objects(&record.children);
        fields.append(&mut flat_fields);
        constraints.append(&mut flat_constraints);
...
fn emit_group_schema(entry: &DataEntry, program: &str) -> Vec<DslFile> {
    ...
    // Build fields and nested objects from COBOL level hierarchy
    let (mut flat_fields, mut flat_constraints, nested_objects) =
        super::schema_fields::build_nested_objects(&entry.children);
    fields.append(&mut flat_fields);
    constraints.append(&mut flat_constraints);
    ...
    if !fields.is_empty() || !nested_objects.is_empty() {
        let schema_file = SchemaFile {
            comments: vec![
                Comment("Generated by cobol2rust Nexflow emitter".to_string()),
                Comment(format!(
...
/// Extract co-access groups from FileHints.
///
/// Groups fields by their `paragraph_scope` -- fields accessed by the same
/// set of paragraphs are semantically related. Returns groups with 3+ fields.
fn extract_co_access_schemas(
    ...
    // Build scope -> fields mapping
    let mut scope_groups: BTreeMap<Vec<String>, Vec<&DataEntry>> = BTreeMap::new();

    for entry in super::cobol_extract::walk_elementary_entries(entries) {
        let snake = super::cobol_extract::cobol_name_to_snake(&entry.name);
        if let Some(hint) = hints.fields.get(&snake) {
    ...
    for (scope, group_entries) in &scope_groups {
        if group_entries.len() < 3 {
            continue;
        }

        // Build schema name from common prefix or first scope paragraph
        let field_names: Vec<String> = group_entries
            .iter()
            .map(|e| super::cobol_extract::cobol_name_to_snake(&e.name))
            .collect();
        let schema_name = common_prefix_name(&field_names)
            .unwrap_or_else(|| {
                let para = scope[0].to_lowercase().replace('-', "_");
                format!("{para}_group")
            });
        ...
        for entry in group_entries {
            ...
            let l88 = super::schema_fields::collect_level88(entry);
            if !l88.is_empty() {
                let name = super::cobol_extract::cobol_name_to_snake(&entry.name);
                constraints.push(SchemaConstraint::Enum(Ident::new(&name), l88));
            }
        }
        ...
        let schema_file = SchemaFile {
            comments: vec![
                Comment("Generated by cobol2rust Nexflow emitter (co-access grouping)".to_string()),
                Comment(format!(
                    "Fields co-accessed by: {}",
                    scope.join(", ")
                )),
                Comment(format!("Source: {program}")),
            ],
            ...
            schemas: vec![SchemaDef {
                name: Ident::new(&schema_name),
                pattern: Some(super::schema_pattern::detect_pattern_for_ws()),
                identity: detect_key_from_flat_fields(&fields),
                fields,
                nested_objects: vec![],
                constraints,
                streaming: None,
        ...
        files.push(DslFile {
            ...
            confidence: 1.0,
            notes: vec![format!(
                "Co-access group: {} fields accessed by {}",
                group_entries.len(),
                scope.join(", ")
            )],
...
/// Find common prefix name from a list of field names.
/// Returns None if no meaningful common prefix.
fn common_prefix_name(names: &[String]) -> Option<String> {
    if names.is_empty() {
        return None;
    }
    let first = &names[0];
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Co-access grouping tests
    // -----------------------------------------------------------------------

    use crate::hints::{FieldHint, FileHints};

    fn make_field_hint(paragraph_scope: Vec<&str>) -> FieldHint {
        FieldHint {
            pic: String::new(),
            usage: String::new(),
            level: 5,
            ...
            read_count: 1,
            write_count: 1,
            paragraph_scope: paragraph_scope.into_iter().map(String::from).collect(),
        }
    }

    fn make_hints_with_scopes(scopes: &[(&str, Vec<&str>)]) -> FileHints {
        let mut fields = std::collections::HashMap::new();
        for (name, scope) in scopes {
            fields.insert(name.to_string(), make_field_hint(scope.clone()));
        }
        FileHints {
            cobol_source: String::new(),
            fields,
            paragraphs: std::collections::HashMap::new(),
            level_88_groups: std::collections::HashMap::new(),
            call_targets: vec![],
            file_io_fields: vec![],
        }
    }
    ...
    #[test]
    fn co_access_groups_same_scope() {
        let entries = vec![
            make_entry("WS-ACCT-ID", "X(10)", 5),
            make_entry("WS-ACCT-NAME", "X(30)", 5),
            make_entry("WS-ACCT-BAL", "9(9)V99", 5),
            make_entry("WS-OTHER", "X(5)", 5),
        ];

        let hints = make_hints_with_scopes(&[
            ("ws_acct_id", vec!["100-VALIDATE", "200-UPDATE"]),
            ("ws_acct_name", vec!["100-VALIDATE", "200-UPDATE"]),
            ("ws_acct_bal", vec!["100-VALIDATE", "200-UPDATE"]),
            ("ws_other", vec!["300-REPORT"]),
        ]);

        let files = extract_co_access_schemas(&entries, &hints, "TESTPROG");
        assert_eq!(files.len(), 1, "Should produce 1 co-access group (3 fields with same scope)");
        let content = &files[0].content;
        assert!(content.contains("co-access"), "Should mention co-access: {content}");
    }

    #[test]
    fn co_access_no_group_when_less_than_3() {
        let entries = vec![
            make_entry("WS-A", "X(10)", 5),
            make_entry("WS-B", "X(10)", 5),
        ];

        let hints = make_hints_with_scopes(&[
            ("ws_a", vec!["100-VALIDATE"]),
            ("ws_b", vec!["100-VALIDATE"]),
        ]);

        let files = extract_co_access_schemas(&entries, &hints, "TESTPROG");
        assert!(files.is_empty(), "Should not group fewer than 3 fields");
    }

    #[test]
    fn no_hints_no_co_access() {
        let program = make_program(vec![
            make_entry("WS-A", "X(10)", 1),
        ]);
        let ctx = DirectEmitterContext {
            cobol_program: &program,
            program_name: "TESTPROG".to_string(),
        ...
        let files = DirectSchemaEmitter.emit(&ctx);
        // No hints -> no co-access files (only structural schemas)
        assert!(
            files.iter().all(|f| !f.content.contains("co-access")),
            "Should have no co-access schemas without hints"
        );
    }
}
