...
impl DslEmitter for ProcessEmitter {
    ...
    fn emit(&self, ctx: &EmitterContext<'_>) -> Vec<DslFile> {
        ...
        let (content, notes, confidence) = generate_process_file(&graph, &program, ctx.target);
...
pub fn generate_process_file(
    graph: &CallGraph,
    program: &str,
    target: Option<&crate::target_config::TargetConfig>,
    ...
    // Derive execution settings from target config
    let (mode, execution) = if let Some(tc) = target {
        use crate::target_config::{AsyncRuntime, IoBackend};
        let mode = if tc.io.backend == IoBackend::Stream {
            ProcessMode::Stream
        } else {
            ProcessMode::Batch
        };
        let exec = if tc.r#async.runtime != AsyncRuntime::Sync {
            Some(ExecutionBlock {
                parallelism: Some(4),
                partition_by: None,
            })
        ...
        if tc.r#async.runtime != AsyncRuntime::Sync {
            notes.push(format!(
                "execution: async runtime = {:?}, scope = {:?}",
                tc.r#async.runtime, tc.r#async.scope
            ));
        }
        (mode, exec)
    } else {
        (ProcessMode::Batch, None)
    };
