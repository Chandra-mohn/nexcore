...
receiveDecl
    : RECEIVE IDENTIFIER (FROM IDENTIFIER)? INTO IDENTIFIER     // receive records from kafka_input into raw_stream
        receiveClause*
    ;
...
keywordOrIdentifier
    ...
    | ACTION        // action is common field name in rule results
    ;
...
transformDecl
    : TRANSFORM IDENTIFIER USING IDENTIFIER INTO IDENTIFIER     // transform input_stream using transform_name into output_stream
        transformOptions?
        embeddedLookup?
        transformStateRef?
...
emitDecl
    : EMIT IDENTIFIER TO sinkName     // emit stream_name to sink_name
        emitClause*
    ;
